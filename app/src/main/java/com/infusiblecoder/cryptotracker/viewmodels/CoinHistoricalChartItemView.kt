...
import com.infusiblecoder.cryptotracker.featurecomponents.historicalchartmodule.HistoricalChartAdapter
import com.infusiblecoder.cryptotracker.api.*
import com.infusiblecoder.cryptotracker.databinding.ActivityHomeBinding
import com.infusiblecoder.cryptotracker.databinding.HistoricalChartModuleBinding
...
class CoinHistoricalChartItemView @JvmOverloads constructor(
    ...
    private lateinit var binding: HistoricalChartModuleBinding

    init {
        // Inflate the layout using View Binding
       binding = HistoricalChartModuleBinding.inflate(LayoutInflater.from(context), this, true)

        // Access views via the binding object
        tvErrorGraph = binding.tvErrorGraph
        pbChartLoading = binding.pbChartLoading
        tvPortfolioChangedValue = binding.tvPortfolioChangedValue
        tvPortfolioChangedPercentage = binding.tvPortfolioChangedPercentage
        tvPortfolioArrow = binding.tvPortfolioArrow
        tvPortfolioChangedDate = binding.tvPortfolioChangedDate
        historicalChartView = binding.historicalChartView
    }
    ...
    private fun animateCoinPrice(amount: String?) {
        if (amount != null) {
            val chartCoinPriceAnimation = ValueAnimator.ofFloat(binding.tvChartCoinPrice.tag.toString().toFloat(), amount.toFloat())
            ...
            chartCoinPriceAnimation.addUpdateListener { updatedAnimation ->
                ...
                binding.  tvChartCoinPrice.text =
                    formatter.formatAmount(animatedValue.toString(), currency)
                binding.  tvChartCoinPrice.tag = animatedValue
            }
    ...
    private fun addRangeSelectorListener(fromCurrency: String) {
        binding. rgPeriodSelector.setOnCheckedChangeListener { _, id ->
            val period = when (id) {
